/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.grupo04.sgpc.service.controller;

import com.grupo04.sgpc.dao.controller.DAOTituloTipo;
import com.grupo04.sgpc.domain.controller.TitulosTipos;
import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

/**
 *
 * @author rafael-d
 */

@Service
public class TituloTipoSI implements TituloTipoService{
    
    @Autowired
    private DAOTituloTipo dao;
    
    @Transactional(readOnly=false)
    @Override
    public void salvar(TitulosTipos tituloTipo) {
        dao.save(tituloTipo);
    }

    @Transactional(readOnly=false)
    @Override
    public void alterar(TitulosTipos tituloTipo) {
        dao.update(tituloTipo);
    }

    @Transactional(readOnly=false)
    @Override
    public void excluir(Long id) {
        dao.delete(id);
    }

    @Transactional(readOnly=true)
    @Override
    public TitulosTipos pesquisarId(Long id) {
        return dao.findById(id);
    }

    @Transactional(readOnly=true)
    @Override
    public List<TitulosTipos> pesquisarTodos() {
        return dao.findAll();
    }

    
}
